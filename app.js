const DATA_URL = "https://arcgis.gdsc.miami.edu/arcgis/rest/services/mdc_public_schools/FeatureServer/1/query?where=1%3D1&outFields=*&returnGeometry=true&outSR=4326&f=geojson";
...
function classify(p) {
  ...
  if (/elementary|elem|primary/.test(text)) return "elementary";
  if (/middle|junior/.test(text)) return "middle";
  if (/senior high|high school|\bhigh\b/.test(text)) return "high";
  return "other";
}
...
function popup(p, lat, lng) {
  ...
  return `<div class="popup"><h3>${safe(p.name || "Public School")}</h3>
    ...
    ${p.enrollmnt ? `<p>Enrollment: ${Number(p.enrollmnt).toLocaleString()}</p>` : ""}
    <a href="${directionsUrl(lat,lng)}" target="_blank" rel="noopener">Open directions →</a></div>`;
}

function normalize(feature) {
  const p = feature.properties || {};
  const coords = feature.geometry?.coordinates || [p.lon, p.lat];
...
function render() {
  ...
  els.status.textContent = visible.length ? `${visible.length} escuelas coinciden con tu búsqueda.` : "No encontramos escuelas con esos filtros.";
}
...
async function loadSchools() {
  try {
    ...
    schools = (data.features || []).map(normalize).filter(Boolean);
    els.total.textContent = schools.length.toLocaleString();
    render();
    if (schools.length) map.fitBounds(L.latLngBounds(schools.map(s => [s.lat,s.lng])), { padding: [30,30] });
  } catch (error) {
    console.error(error);
    els.status.innerHTML = "No se pudieron cargar los datos. Revisa tu conexión y recarga la página.";
  }
}
...
els.locate.addEventListener("click", () => {
  if (!navigator.geolocation) return (els.status.textContent = "Tu navegador no permite geolocalización.");
  els.status.textContent = "Buscando tu ubicación…";
  navigator.geolocation.getCurrentPosition(({coords}) => {
    const here = [coords.latitude, coords.longitude];
    if (userMarker) map.removeLayer(userMarker);
    userMarker = L.circleMarker(here, { radius: 9, weight: 4, color: "#fff", fillColor: "#172033", fillOpacity: 1 }).addTo(map).bindPopup("Tu ubicación").openPopup();
    map.setView(here, 13); els.status.textContent = "Mostrando las escuelas cercanas a tu ubicación.";
  }, () => { els.status.textContent = "No pudimos obtener tu ubicación. Verifica el permiso del navegador."; }, { enableHighAccuracy: true, timeout: 10000 });
});
